...
$r = $con->query("select * from addgame where genre_id= '$genre_id' AND gamename LIKE '%$keyword%'"); 
...
<script type='text/javascript'>
    ...
    <main>
        ...
        $sqll = "SELECT * from gamecate where id = $genre_id ";
        $resulttt = mysqli_query($con, $sqll);
        $groww = mysqli_fetch_assoc($resulttt);
        ?>
        <h1 style="text-align: center; padding: 50px; ">
           
            <?php echo "Search results for keyword:- ".$keyword." and genre:- ".$groww['name'].""; ?>
