...
if(isset($_SESSION['id'])  ){
    ...
    $r = $con->query("select * from addgame a INNER JOIN z_orders z on z.game_id = a.id");
    
    
    
    while ($row = $r->fetch_array()) { 
    // $rrr = $con->query("select * from cart where id = '.$row[2].'");
    // $brow = $rrr->fetch_array();
    // $rr = $con->query("select * from addgame where id= ".$brow[2]."");
    // $bbrow = $rr->fetch_array();
                //   $query_run = mysqli_query($con, $query);
                //   $roww=$query_run->fetch_row();
                echo ' <div class="cart-item">
                <img src="../admin_login/upload/'.$row[7].' " alt="Product 1" style="width: 300px;">
                <div class="cart-item-details">
                  <p><strong>'.$row[1].' </strong></p>
                  <p>Price: '.$row[6].'</p>
                  <p><strong>Description:</strong>  '.$row[5].' </p>
                  <p><strong>Genre:</strong>  '.$row[2].'</p>
                  <p><strong>Release Date:</strong> '.$row[4].'</p>
                  <button class ="btn btn-danger"><a href="../admin_login/upload/'.$row[10].'"class ="text-light"> Download</a></button>
                </div>
              </div>';
              }
              }
